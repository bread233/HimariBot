...
from ..localstore import get_plugin_data_file
...
driver = get_driver()
plugin_data_file: Path = get_plugin_data_file("limiter_data.json")
...
class PersistData(BaseModel):
    ...
    fix_window: Optional[dict[str, dict[str, "FixWindowSet"]]] = None

    class SlidingWindowSet(BaseModel):
        timestamps: list[int]

    sliding_window: Optional[dict[str, dict[str, "SlidingWindowSet"]]] = None
    ...
    leaky_bucket: Optional[dict[str, dict[str, "LeakyBucketSet"]]] = None
    ...
    token_bucket: Optional[dict[str, dict[str, "TokenBucketSet"]]] = None
...
def save_usage_data() -> None:
    """保存用量数据到本地"""
    j = {"fix_window": {}, "sliding_window": {}, "leaky_bucket": {}, "token_bucket": {}}
