...
group_add = on_command(
    "添加", permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER, priority=15, block=True
)
group_remove = on_command(
    "移除", permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER, priority=15, block=True
)
...
remove_greeting = on_command(
    "删除问候",
    aliases={"删除问候语", "移除问候", "移除问候语"},
    permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER,
    priority=12,
    block=True,
)
    ...
    if is_too_many_lines:
        await bot.call_api(
            ...
            messages=MessageSegment.node_custom(
                int(bot.self_id), list(bot.config.nickname)[0], msg
            ),
        )
    else:
...
def parse_greeting() -> Coroutine[Any, Any, None]:
    ...
    async def _greeting_parser(
        matcher: Matcher, state: T_State, input_arg: Message = Arg("greeting")
    ) -> None:
...
def parse_meal() -> Coroutine[Any, Any, None]:
    ...
    async def _meal_parser(
        matcher: Matcher, state: T_State, input_arg: str = ArgStr("meal")
    ) -> None:
        if input_arg == "取消":
            await matcher.finish("操作已取消")
...
def parse_index() -> None:
    ...
    async def _index_parser(
        matcher: Matcher, state: T_State, input_arg: str = ArgStr("index")
    ) -> None:
        try:
...
@add_greeting.got(
    "greeting", prompt="请输入添加的问候语，输入取消以取消操作", parameterless=[Depends(parse_greeting())]
)
...
@remove_greeting.got(
    "index", prompt="请输入删除的问候语序号，输入0以取消操作", parameterless=[Depends(parse_index())]
)
