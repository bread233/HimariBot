...
async def live():  # 定时推送直播间状态
    ...
    if live_index == len(anchors):
        return
    status, live_status, content = await data_source.LiveRoomInfo(
        anchors[live_index][0]
    )
        ...
        if card[3] == 1:  # 允许推送直播
            if card[1] == 1:  # 是群聊
                if card[4] == 1:  # 需要@全体成员
                    ...
                    await schedBot.call_api(
                        "send_msg", **{"message": content, "group_id": card[0]}
                    )
            else:  # 私聊
                await schedBot.call_api(
                    "send_msg", **{"message": content, "user_id": card[0]}
                )
    live_index += 1
...
async def dynamic():  # 定时推送最新用户动态
    ...
    status, dynamic_id, content = await data_source.LatestDynamicInfo(
        anchors[dynamic_index][0]
    )
