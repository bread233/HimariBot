...
def Init():
    ...
    CUR = DB.cursor()
    CUR.execute(
        'select count(*) from sqlite_master where type="table" and name = "anchor_list"'
    )
...
def AddNewAnchor(Mid: str, anchor: str, live_exist: int):  # 创建主播对应的表
    ...
    if CUR.fetchall()[0][0] == 0:
        CUR.execute(
            "create table {} (id TEXT,is_group INTEGER,dynamic INTEGER,live INTEGER,at INTEGER)".format(
                Name
            )
        )
        CUR.execute(
            'insert into anchor_list values("{}","{}","",{},0)'.format(
                Mid, anchor, live_exist
            )
        )
        DB.commit()
    else:
...
def AddCard(
    Mid: str, ID: str, group: int, Live: int = 1
) -> int:  # 添加主播信息 返回类型 记录是否已存在(int)1：存在
    DB = sqlite3.connect("bilibili.db")
    CUR = DB.cursor()
    Name = "_" + Mid
    CUR.execute(
        'select count(*) from {} where id="{}" and is_group={}'.format(Name, ID, group)
    )
    if CUR.fetchall()[0][0] != 0:
    ...
    CUR.execute(
        'insert into {} values("{}",{},{},{},{})'.format(
            Name, ID, str(group), str(1), str(Live), str(0)
        )
    )
    DB.commit()
    CUR.close()
...
def GetCard(Mid: str, ID: str, group: int):  # 获取主播信息
    ...
    CUR = DB.cursor()
    CUR.execute(
        'select * from {} where id="{}" and is_group={}'.format(Name, ID, group)
    )
    data = CUR.fetchall()
    if len(data) == 0:
        CUR.close()
...
def DynamicON(Mid: str, ID: str, group: int):
    ...
    CUR = DB.cursor()
    CUR.execute(
        'update {} set dynamic=1 where id="{}" and is_group={}'.format(Name, ID, group)
    )
    DB.commit()
    CUR.close()
    DB.close()
...
def DynamicOFF(Mid: str, ID: str, group: int):
    ...
    CUR = DB.cursor()
    CUR.execute(
        'update {} set dynamic=0 where id="{}" and is_group={}'.format(Name, ID, group)
    )
    DB.commit()
    CUR.close()
    DB.close()
...
def LiveON(Mid: str, ID: str, group: int):
    ...
    CUR = DB.cursor()
    CUR.execute(
        'update {} set live=1 where id="{}" and is_group={}'.format(Name, ID, group)
    )
    DB.commit()
    CUR.close()
    DB.close()
...
def LiveOFF(Mid: str, ID: str, group: int):
    ...
    CUR = DB.cursor()
    CUR.execute(
        'update {} set live=0 where id="{}" and is_group={}'.format(Name, ID, group)
    )
    DB.commit()
    CUR.close()
    DB.close()
...
def AtON(Mid: str, ID: str, group: int):
    ...
    CUR = DB.cursor()
    CUR.execute(
        'update {} set at=1 where id="{}" and is_group={}'.format(Name, ID, group)
    )
    DB.commit()
    CUR.close()
    DB.close()
...
def AtOFF(Mid: str, ID: str, group: int):
    ...
    CUR = DB.cursor()
    CUR.execute(
        'update {} set at=0 where id="{}" and is_group={}'.format(Name, ID, group)
    )
    DB.commit()
    CUR.close()
    DB.close()
...
def UpdateLive(mid: str, status: int):
    ...
    CUR = DB.cursor()
    CUR.execute(
        'update anchor_list set live_status={} where id="{}"'.format(status, mid)
    )
    DB.commit()
    CUR.close()
    DB.close()
...
def UpdateDynamic(mid: str, dynamicid: str):
    ...
    CUR = DB.cursor()
    CUR.execute(
        'update anchor_list set dynamic_id="{}" where id="{}"'.format(dynamicid, mid)
    )
    DB.commit()
    CUR.close()
    DB.close()
