...
class BiliClient:
    ...
    async def get_latest_video(
        self, client: httpx.AsyncClient, uid: str, last_udpate_time: int
        ...
        try:
            ...
            response = await client.get(
                url=self.API["get_latest_video_by_uid"], params=self.wbi_sign(params)
            )
        except Exception as e:
        ...
        async with httpx.AsyncClient(headers=self.headers) as client:
            try:
                response1 = await client.get(
                    url=self.API["get_user_info_by_uid"],
                    params=self.wbi_sign({"mid": uid}),
                )
            except Exception as e:
                raise BiliConnectionError(0, uid, e.args[0])
            ...
            try:
                response2 = await client.get(
                    url=self.API["get_latest_video_by_uid"],
    ...
    async def get_live_status(
        self, client: httpx.AsyncClient, uid: str, room_id: str
    ) -> Tuple[bool, str, str]:
        """
        @description  :
        根据房间号,获取直播间是否开播
        ...
        try:
            response = await client.get(
                self.API["get_live_info_by_room_id"], params={"room_id": room_id}
            )
        except Exception as e:
    ...
    async def init_liver_info(self, uid: str) -> Tuple[str, str]:
        ...
        async with httpx.AsyncClient(headers=self.headers) as client:
            try:
                response = await client.get(
                    self.API["get_liver_info_by_uid"], params={"uid": uid}
                )
            except Exception as e:
                raise BiliConnectionError(0, uid, e.args[0])
    ...
    async def init_liver_info_by_room_id(self, room_id: str) -> Tuple[str, str]:
        ...
        async with httpx.AsyncClient(headers=self.headers) as client:
            try:
                response1 = await client.get(
                    url=self.API["get_live_info_by_room_id"],
    ...
    async def init_telegram_info_by_ep_id(
        ...
        async with httpx.AsyncClient(headers=self.headers) as client:
            try:
                response = await client.get(
                    url=self.API["get_telegram_info_by_ep_id"], params={"ep_id": ep_id}
                )
            except Exception as e:
    ...
    async def init_telegram_info_by_season_id(
        ...
        async with httpx.AsyncClient(headers=self.headers) as client:
            try:
                response = await client.get(
                    url=self.API["get_telegram_info_by_season_id"],
    ...
    async def init_telegram_info_by_media_id(
        ...
        async with httpx.AsyncClient(headers=self.headers) as client:
            try:
                response = await client.get(
                    url=self.API["get_telegram_info_by_media_id"],
    ...
    async def get_telegram_latest_episode(
        self, client: httpx.AsyncClient, season_id: int, index: int
        ...
        try:
            response = await client.get(
                url=self.API["get_telegram_latest_episode"],
                params={"season_id": season_id},
            )
        except Exception as e:
            raise BiliConnectionError(2, f"season_id:{season_id}", e.args[0])
    ...
    def update_dynamic_cookie(self) -> None:
        ...
        with httpx.Client(headers=headers) as client:
            ...
            json_data = {"payload": json.dumps(data, separators=(",", ":"))}
...
scheduler.add_job(
    bili_client.update_dynamic_cookie,
    "interval",
    hours=12,
    id="update_dynamic_cookie",
    misfire_grace_time=90,
)
scheduler.add_job(
    bili_client.get_img_key_and_sub_key,
    "interval",
    minutes=10,
    id="update_img_sub_key",
    misfire_grace_time=90,
)
