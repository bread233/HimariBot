...
def Anosu(N: int = 1, Tag: str = "", R18: int = 0):
    ...
    if 1 <= N <= 10:
        ...
        if R18:
            ...
            msg += f"Bot_NICKNAME被禁止单次发送超过10张色图...但是，Bot_NICKNAME为你准备了一张随机{Tag}色图。"
    else:
        N = 1
        msg += (
            f"Bot_NICKNAME接收到了奇怪的数量参数，不过Bot_NICKNAME送你一张随机{'r18'if R18 else ''}{Tag}色图。"
        )
    logger.info(f"正在从 anosu 获取图片。")
    resp = httpx.get(
        f"https://image.anosu.top/pixiv/json?num={N}&r18={R18}&keyword={Tag}"
    )
