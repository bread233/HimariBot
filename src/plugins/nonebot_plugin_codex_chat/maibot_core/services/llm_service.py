    ...
    async def generate_response_with_messages(
        ...
        async def _resolve_messages() -> list[Message]:
            if len(inspect.signature(message_factory).parameters) >= 2:
                message_result = message_factory(None, None)
            else:
                message_result = message_factory(None)
            return await message_result if inspect.isawaitable(message_result) else message_result

        messages = await _resolve_messages()
        prompt_text = self._build_cache_stats_prompt_text(
            messages=messages,
            tool_options=active_options.tool_options,
            response_format=active_options.response_format,
