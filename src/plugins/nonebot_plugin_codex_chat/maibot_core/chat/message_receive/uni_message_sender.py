...
from ...common.message_server.api import get_global_api
from ...common.utils.utils_message import MessageUtils
from ...webui.routers.chat.serializers import serialize_message_sequence
...
async def send_prepared_message_to_platform(message: SessionMessage, show_log: bool = True) -> bool:
    ...
    若当前处于 outbound capture 上下文，则只记录而不真正外发。
    """

    if has_outbound_capture():
        capture_outbound_message(
            text=str(getattr(message, "processed_plain_text", "") or ""),
            reply_to_message_id=str(getattr(message, "reply_to", "") or "").strip() or None,
            raw=message,
        )
        if show_log:
            logger.info("[outbound-capture] 已捕获消息，跳过真实外发")
        return True
