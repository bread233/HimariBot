...
from ..core.tooling import ToolAvailabilityContext, ToolRegistry
from ..llm_models.model_client.base_client import BaseClient
...
from ..llm_models.payload_content.tool_option import ToolCall, ToolDefinitionInput, ToolOption, normalize_tool_options
from ..plugin_runtime.hook_payloads import (
...
from ..plugin_runtime.hook_schema_utils import build_object_schema
from ..plugin_runtime.host.hook_spec_registry import HookSpec, HookSpecRegistry
    ...
    async def chat_loop_step(
        ...
        *,
        ...
        generation_result = await llm_chat.generate_response_with_messages(
            ...
            latest_at_current_bot_msg_id=latest_at_bot,
        )
        ...
        *,
        ...
        for context_message in reversed(chat_history or []):
            if not isinstance(context_message, SessionBackedMessage):
                continue
            if context_message.source_kind not in ("user", "injected"):
                continue
            ...
            if not bot_self_id:
                continue
            for comp in context_message.raw_message.components:
                if isinstance(comp, AtComponent) and comp.target_user_id == bot_self_id:
                    return msg_id.strip()
        return None
