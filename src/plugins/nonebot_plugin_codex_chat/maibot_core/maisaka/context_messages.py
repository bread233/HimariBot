...
def _append_emoji_component(
    ...
    *,
    ...
    builder.add_text_content("[表情:视觉识别失败，内容未知]")
    return True
...
def _append_image_component(
    ...
    *,
    ...
    builder.add_text_content("[图片:视觉识别失败，内容未知]")
    return True
...
def _render_component_for_prompt(component: StandardMessageComponents) -> str:
    ...
    if isinstance(component, ImageComponent):
        return component.content.strip() if component.content else "[图片:视觉识别失败，内容未知]"

    if isinstance(component, EmojiComponent):
        return component.content.strip() if component.content else "[表情:视觉识别失败，内容未知]"
