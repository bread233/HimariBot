...
from .retrieval import build_retrieval_context, score_text_overlap
...
def _needs_reliable_context(prompt: str) -> bool:
    ...
    return any(
        ...
        for token in [
            '我是谁', '我叫什么', '我在群里叫什么',
            '刚才', '刚刚', '之前', '说了什么', '在测什么',
            '什么', '多少', '多少钱', '价格', '参数', '配置', '规格',
            '显存', '内存', '发布', '发售', '最新', '现在', '当前',
            '属于', '系列', '支持', '区别', '对比', '是真的吗',
            '有吗', '存在', '查', '搜索', '资料',
        ]
    )
...
async def build_context_pack(config, session_info: dict, prompt: str, bot=None, event=None) -> dict:
    ...
    if is_identity_question:
        ...
        if retrieval_score >= retrieval_threshold and retrieval_context:
            tool_notes.append('retrieval_source=db')
            tool_notes.append(f'retrieval_score={retrieval_score:.2f}')
            if should_web and web_enabled and route_like:
                try:
