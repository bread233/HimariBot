...
from src.service.apscheduler import scheduler  # noqa:E402
...
connect_core = on_fullmatch(("连接core", "链接core"), permission=SUPERUSER, block=True)
...
async def get_all_message(bot: Bot, ev: Event):
    ...
    # kaiheila
        ...
        if isinstance(ev, ChannelMessageEvent) or isinstance(ev, PrivateMessageEvent):
    ...
    # onebot
        ...
        if isinstance(ev, GroupEventMessage) or isinstance(ev, PrivateEventMessage):
        ...
        if isinstance(ev, TextMessageEvent):
            ...
            if "from_wxid" in ev.data:
                if "raw_msg" in ev.data and "xml" in ev.data["raw_msg"]:
                    match = re.search(r"<svrid>(\d+)</svrid>", ev.data["raw_msg"])
                    if match:
...
def convert_message(_msg: Any, message: List[Message], index: int):
    if _msg.type == "text":
        data: str = _msg.data["text"] if "text" in _msg.data else _msg.data["content"]
