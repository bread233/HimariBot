...
from src.service.nonebot_plugin_saa import Image, Text, TargetQQGroup, Mention, AggregatedMessageFactory, enable_auto_select_bot
from src.service.nonebot_plugin_orm import AsyncSession, async_scoped_session, get_session, get_scoped_session
from src.service.apscheduler import scheduler
from src.service.nonebot_plugin_limiter import UserScope, Cooldown, GlobalScope, Increaser


driver = get_driver()
# 创建依赖注入对象
# 使用 get_scoped_session()() 作为 Depends 的返回值工厂，避免直接传入 get_scoped_session 导致 DI 解析问题
async_session_dep = Depends(lambda: get_scoped_session()())
...
async def _(event: MessageEvent, session: AsyncSession = async_session_dep):
    try:
...
async def format_record_message(record_data: dict, user_name: str) -> Union[bytes, str, None]:
    """格式化战绩播报消息"""
    try:
        # 解析时间
        event_time = record_data.get('dtEventTime', '')
...
async def format_tdm_record_message(record_data: dict, user_name: str) -> Union[bytes, str, None]:
    """格式化战场战绩播报消息"""
    try:
        # 解析时间
        event_time = record_data.get('dtEventTime', '')
...
async def safehouse_remind_open_close(event: MessageEvent, session: AsyncSession = async_session_dep, args: Message = CommandArg()):
...
async def broadcast_record_open_close(event: MessageEvent, session: AsyncSession = async_session_dep, args: Message = CommandArg()):
    user_data_database = UserDataDatabase(session)
    user_data = await user_data_database.get_user_data(event.user_id)
    if not user_data:
...
async def _(event: MessageEvent, session: AsyncSession = async_session_dep, args: Message = CommandArg()):
...
async def _(event: MessageEvent, increaser: Increaser, session: AsyncSession = async_session_dep):
    user_data_database = UserDataDatabase(session)
    user_data = await user_data_database.get_user_data(event.user_id)
    if user_data:
        access_token = user_data.access_token
...
async def get_record(event: MessageEvent, session: AsyncSession = async_session_dep, args: Message = CommandArg()):
    user_data_database = UserDataDatabase(session)
    user_data = await user_data_database.get_user_data(event.user_id)
    if not user_data:
...
async def watch_record(user_name: str, qq_id: int):
    # 使用 get_session() — 确保在插件初始化时已调用 init_orm(plugin)
    session = get_session()
    user_data_database = UserDataDatabase(session)
                ...
                if not latest_record_data or latest_record_data.latest_tdm_record_id != record_id:
                    ...
                    try:
                        if result:
                            ...
                            if await user_data_database.update_latest_record(latest_record_data):
                                await user_data_database.commit()
                                logger.info(f"更新最新戰绩记录成功: {user_name} - {record_id}")
                            else:
                                logger.error(f"更新最新戰绩记录失败: {user_name} - {record_id}")
...
async def initialize_plugin():
    ...
    # 尝试初始化 ORM（需要传入插件对象）
    try:
        from src.service.nonebot_plugin_orm import init_orm
        from nonebot.plugin import get_plugin_by_module_name
        plugin = get_plugin_by_module_name(__name__) or get_plugin_by_module_name("nonebot_plugin_delta_helper")
        if plugin:
            init_orm(plugin)
            logger.info("ORM 已为三角洲插件初始化")
        else:
            logger.warning("无法获取插件对象以初始化 ORM，后续数据库调用可能失败")
    except Exception as e:
        logger.exception(f"初始化 ORM 失败: {e}")

    # 启动战绩监控（以后台任务方式启动，避免阻塞启动流程）
    try:
        asyncio.create_task(start_watch_record())
    except Exception as e:
        logger.exception(f"启动战绩监控任务失败: {e}")
