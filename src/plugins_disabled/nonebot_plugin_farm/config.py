...
# PROJECT_ROOT: 项目根目录 /home/xmb/bot/Himaribot-dev
PROJECT_ROOT = Path(__file__).resolve().parents[3]

# DATA_ROOT: 根目录下的 data 目录 /home/xmb/bot/Himaribot-dev/data
DATA_ROOT = PROJECT_ROOT / "data"

# 农场数据库目录: /home/xmb/bot/Himaribot-dev/data/nonebot_plugin_farm/farm_db
g_sDBPath = DATA_ROOT / "nonebot_plugin_farm" / "farm_db"
g_sDBPath.mkdir(parents=True, exist_ok=True)

# 农场数据库文件: /home/xmb/bot/Himaribot-dev/data/nonebot_plugin_farm/farm_db/farm.db
g_sDBFilePath = g_sDBPath / "farm.db"

# 农场资源文件目录（保持原路径：插件内部 resource 目录）
g_sResourcePath = Path(__file__).resolve().parent / "resource"

# 农场作物数据库（插件内部 db/plant.db）
g_sPlantPath = g_sResourcePath / "db/plant.db"

# 农场配置文件目录（插件内部 config 目录）
